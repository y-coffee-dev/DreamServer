...
if command -v jq >/dev/null 2>&1; then
    if jq empty "$TEMP_REPORT" 2>/dev/null; then
...
if command -v jq >/dev/null 2>&1; then
    ...
    for field in "${required_fields[@]}"; do
        if ! jq -e ".$field" "$TEMP_REPORT" >/dev/null 2>&1; then
...
if command -v jq >/dev/null 2>&1; then
    if jq -e '.autofix_hints | type == "array"' "$TEMP_REPORT" >/dev/null 2>&1; then
...
if command -v jq >/dev/null 2>&1; then
    ...
    for field in "${runtime_fields[@]}"; do
        field_type=$(jq -r ".runtime.$field | type" "$TEMP_REPORT" 2>/dev/null)
...
if command -v jq >/dev/null 2>&1; then
    ...
    for field in "${summary_fields[@]}"; do
        field_type=$(jq -r ".summary.$field | type" "$TEMP_REPORT" 2>/dev/null)
...
# 10. Script is executable or runnable via bash
